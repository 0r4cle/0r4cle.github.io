...
import {
    amosNavigationItem,
    amosVerticalNavigationAppearance,
    amosVerticalNavigationMode,
    amosVerticalNavigationPosition,
} from '@amos/components/navigation/navigation.types';
...
@Component({
    selector: 'amos-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: amosAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'amosVerticalNavigation',
})
export class amosVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: amosVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: amosVerticalNavigationMode = 'side';
    @Input() name: string = this._amosUtilsService.randomId();
    @Input() navigation: amosNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: amosVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<amosVerticalNavigationAppearance> =
        new EventEmitter<amosVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<amosVerticalNavigationMode> =
        new EventEmitter<amosVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<amosVerticalNavigationPosition> =
        new EventEmitter<amosVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<amosNavigationItem> =
        new ReplaySubject<amosNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<amosNavigationItem> =
        new ReplaySubject<amosNavigationItem>(1);
    ...
    private _amosScrollbarDirectives!: QueryList<amosScrollbarDirective>;
    private _amosScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        ...
        return {
            ...
            [`amos-vertical-navigation-appearance-${this.appearance}`]: true,
            'amos-vertical-navigation-hover': this._hovered,
            'amos-vertical-navigation-inner': this.inner,
            'amos-vertical-navigation-mode-over': this.mode === 'over',
            'amos-vertical-navigation-mode-side': this.mode === 'side',
            'amos-vertical-navigation-opened': this.opened,
            'amos-vertical-navigation-position-left': this.position === 'left',
    ...
    /**
     * Setter for amosScrollbarDirectives
     */
    @ViewChildren(amosScrollbarDirective)
    set amosScrollbarDirectives(
        amosScrollbarDirectives: QueryList<amosScrollbarDirective>
    ) {
        // Store the directives
        this._amosScrollbarDirectives = amosScrollbarDirectives;

        // Return if there are no directives
        if (amosScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._amosScrollbarDirectivesSubscription) {
            this._amosScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._amosScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                amosScrollbarDirectives.forEach((amosScrollbarDirective) => {
                    amosScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        setTimeout(() => {
            ...
            // Otherwise
                ...
                this._amosScrollbarDirectives.forEach(
                    (amosScrollbarDirective) => {
                        // Skip if not enabled
                        if (!amosScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        amosScrollbarDirective.scrollToElement(
                            '.amos-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: amosNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: amosNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('amos-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'amos-vertical-navigation-overlay-transparent'
            );
        }
