...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@amos/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[amosScrollbar]',
    exportAs: 'amosScrollbar',
})
export class amosScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_amosScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() amosScrollbar: boolean = true;
    @Input() amosScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('amosScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.amosScrollbar = coerceBooleanProperty(
                changes.amosScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.amosScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('amosScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.amosScrollbarOptions.currentValue
            );
